...
class ConditionalDensityEstimator(abc.ABC):
    ...
    def __init__(self, param_dim: int, feature_dim: int):
        if not (param_dim > 0 and feature_dim >= 0):
            raise ValueError(
                "Parameter and feature dimensions must be positive/non-negative."
            )
        ...
        for epoch in iterator:
            ...
            for i in range(0, n_train, batch_size):
                ...
                for key in self.weights:
                    ...
                    self.weights[key] -= (
                        learning_rate * m_hat / (anp.sqrt(v_hat) + epsilon)
                    )

            # Validation step
            avg_train_loss = anp.mean(epoch_train_losses)
            val_loss = self._loss_function(
                self.weights, features[val_idx], params[val_idx]
            )
            ...
            if use_tqdm:
                iterator.set_postfix(
                    train=f"{avg_train_loss:.3f}", val=f"{val_loss:.3f}"
                )
...
class MAFEstimator(ConditionalDensityEstimator):
    ...
    def _made_forward(self, y, ctx, layer_const, k, weights):
        ...
        mu, alpha = (
            out[:, : self.param_dim],
            anp.clip(out[:, self.param_dim :], -7.0, 7.0),
        )
        return mu, alpha
...
def run_test(
    estimator: ConditionalDensityEstimator, dataset_name: str, plot: bool = True
):
    """
    ...
    grid_x, grid_y = anp.meshgrid(
        anp.linspace(*x_range, 100), anp.linspace(*y_range, 100)
    )
    ...
    ax2.contourf(
        grid_x, grid_y, anp.exp(log_p_high), levels=10, cmap="Oranges", alpha=0.8
    )
    ax2.plot(
        samples_high[:, 0],
        samples_high[:, 1],
    ...
    fig.suptitle(
        f"Density Estimation Results for {estimator.__class__.__name__} on '{dataset_name}'",
        fontsize=16,
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Train and test conditional density estimators"
    )
    ...
    if args.test:
        ...
        mdn = MDNEstimator(
            param_dim=2, feature_dim=1, n_components=3, hidden_sizes=(16, 16)
        )
        ...
        # --- MDN Tests ---
        mdn_banana = MDNEstimator(
            param_dim=2, feature_dim=1, n_components=8, hidden_sizes=(64, 64)
        )
        run_test(mdn_banana, "banana", plot)

        mdn_moons = MDNEstimator(
            param_dim=2, feature_dim=1, n_components=10, hidden_sizes=(64, 64)
        )
        run_test(mdn_moons, "moons", plot)

        # --- MAF Tests ---
        maf_banana = MAFEstimator(
            param_dim=2, feature_dim=1, n_flows=5, hidden_units=128
        )
        run_test(maf_banana, "banana", plot)

        maf_moons = MAFEstimator(
            param_dim=2, feature_dim=1, n_flows=5, hidden_units=128
        )
