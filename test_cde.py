...
def _find_lib():
    ...
    candidates = list(REPO_ROOT.glob("libsmolmaf.so")) + list(
        REPO_ROOT.glob("**/libsmolmaf.so")
    )
    for p in candidates:
        if p.exists():
            return str(p)
...
def _find_cli():
    ...
    candidates = [REPO_ROOT / "build" / "smolcde", REPO_ROOT / "smolcde"]
    for p in candidates:
...
def test_mdn_basic():
    ...
    assert samples.shape == (1, 10, 2), (
        f"Expected shape (1, 10, 2), got {samples.shape}"
    )
    ...
    assert anp.all(anp.isfinite(log_probs)), (
        "Log probabilities contain non-finite values"
    )
...
def test_data_generation():
    ...
    for dataset in datasets:
        ...
        assert params.shape == (50, 2), (
            f"Expected params shape (50, 2), got {params.shape}"
        )
        assert features.shape == (50, 1), (
            f"Expected features shape (50, 1), got {features.shape}"
        )
        assert not anp.any(anp.isnan(params)), (
            f"Params contain NaN for dataset {dataset}"
        )
        assert not anp.any(anp.isnan(features)), (
            f"Features contain NaN for dataset {dataset}"
        )
...
def test_lorenz():
    ...
    model = MAFEstimator(
        n_flows=4, hidden_units=64, param_dim=3, feature_dim=features.shape[1]
    )
    model.train(
        Y_train,
        X_train,
...
class MAF_C_Trainer:
    def __init__(self, lib_path):
        ...
        # Core API
        self.lib.maf_init_random_model.argtypes = [
            ctypes.c_uint16,
            ctypes.c_uint16,
            ctypes.c_uint16,
            ctypes.c_uint16,
        ...
        self.lib.maf_create_adam.argtypes = [
            ctypes.c_void_p,
            ctypes.c_float,
            ctypes.c_float,
            ctypes.c_float,
        ...
        self.lib.maf_forward_train.argtypes = [
            ctypes.c_void_p,
            ctypes.c_void_p,
            ctypes.c_void_p,
        ...
        self.lib.maf_backward.argtypes = [
            ctypes.c_void_p,
            ctypes.c_void_p,
            ctypes.c_void_p,
            ctypes.POINTER(ctypes.c_float),
        ...
        self.lib.maf_adam_step.argtypes = [
            ctypes.c_void_p,
            ctypes.c_void_p,
            ctypes.c_void_p,
        ]
...
def test_c_training():
    ...
    py_model.train(
        params, features, n_epochs=50, batch_size=8, learning_rate=0.001, use_tqdm=False
    )
    test_feat = np.array([0.5], dtype=np.float32)
    n_samples = 1000
    c_samples = trainer.sample(c_model, test_feat, n_samples)
    py_samples = py_model.sample(
        test_feat.reshape(1, -1), n_samples, np.random.RandomState(42)
    ).reshape(-1, 2)
...
def test_c_gradient_matches_autograd():
    ...
    py_model.train(
        params, features, n_epochs=1, learning_rate=0.001, use_tqdm=False, seed=99
    )
    ...
    W1c = (
        w["W1c_0"].astype(np.float32).flatten()
        if C > 0
        else np.zeros(H * C, dtype=np.float32)
    )
    b1 = w["b1_0"].astype(np.float32).flatten()
    W2 = w["W2_0"].astype(np.float32).flatten()
    W2c = (
        w["W2c_0"].astype(np.float32).flatten()
        if C > 0
        else np.zeros(2 * D * C, dtype=np.float32)
    )
...
def test_c_deterministic_sampling():
    ...
    np.testing.assert_array_equal(
        samples_out1, samples_out2, "Deterministic sampling not reproducible"
    )
...
def test_model_serialization_roundtrip(tmp_path):
    ...
    assert len(mean_rows) == len(features), (
        f"Expected {len(features)} mean rows, got {len(mean_rows)}"
    )
...
def test_infer_all_modes(tmp_path):
    ...
    with open(out_sample, "r") as f:
        reader = csv.DictReader(f)
        rows = list(reader)
    assert len(rows) == len(features) * 8, (
        f"Sample mode: expected {len(features) * 8} rows"
    )
    ...
    assert len(rows) == len(features) * 2, (
        f"Stats mode: expected {len(features) * 2} rows"
    )
    ...
    with open(out_quant, "r") as f:
        ...
        rows = list(reader)
    assert len(rows) == len(features) * 2, (
        f"Quantiles mode: expected {len(features) * 2} rows"
    )
...
def test_skip_header(tmp_path):
    ...
    assert result.returncode == 0, (
        f"Training with --skip-header failed: {result.stderr}"
    )
    ...
    assert result.returncode == 0, (
        f"Inference with --skip-header failed: {result.stderr}"
    )
...
def test_c_log_prob():
    ...
    c_weights.perm_data = arr(layer["perm"].astype(np.uint16)).ctypes.data_as(
        ctypes.POINTER(ctypes.c_uint16)
    )
    c_weights.inv_perm_data = arr(layer["inv_perm"].astype(np.uint16)).ctypes.data_as(
        ctypes.POINTER(ctypes.c_uint16)
    )
    ...
    c_logp = trainer.lib.maf_log_prob(
        c_model,
        ws,
        test_f.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
        test_p.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
    ...
    py_logp_sum = np.sum(
        py_model.log_prob(
            anp.array(test_f.astype(np.float32)), anp.array(test_p.astype(np.float32))
        )
    )
...
def test_mnist_mdn_accuracy():
    ...
    mdn = MDNEstimator(
        param_dim=1, feature_dim=64, n_components=5, hidden_sizes=(64, 32)
    )
    mdn.train(
        y_noisy, X_train, n_epochs=300, learning_rate=0.005, use_tqdm=False, seed=42
    )
...
def test_c_backward_feature_dim_zero():
    ...
    lib.maf_forward_train.argtypes = [ctypes.c_void_p] * 3 + [
        ctypes.POINTER(ctypes.c_float)
    ] * 2
    lib.maf_forward_train.restype = ctypes.c_float
    lib.maf_backward.argtypes = [ctypes.c_void_p] * 3 + [
        ctypes.POINTER(ctypes.c_float)
    ] * 2
...
def test_cli_bad_model_file(tmp_path):
    ...
    assert "error" in combined, (
        f"Expected error message, got: {result.stderr}{result.stdout}"
    )
    assert result.returncode != 0, (
        f"Expected non-zero exit for bad model, got {result.returncode}"
    )
    ...
    combined = (result.stderr + result.stdout).lower()
    assert "error" in combined, (
        f"Expected error for invalid magic, got: {result.stderr}"
    )
    assert result.returncode != 0, (
        f"Expected non-zero exit for bad magic, got {result.returncode}"
    )
